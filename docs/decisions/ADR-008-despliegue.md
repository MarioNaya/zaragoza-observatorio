# ADR-008 — Despliegue: imagen Docker propia sobre Railway con la plantilla PostGIS
...
4. **La base de datos es la plantilla PostGIS (`postgis/postgis:17-3.5`)**, no el Postgres por defecto. Con ello la misma imagen de PostgreSQL corre en desarrollo (`compose.yaml`), en los tests (Testcontainers) y en producción: las diferencias de versión o de extensiones dejan de ser una fuente posible de fallo.
...
8. **La infraestructura se declara en `.railway/railway.ts` (Infrastructure as Code).** `railway.json` está deprecado y cerrado a servicios nuevos, así que escribirlo sería introducir algo que deja de leerse el 2026-12-01; el sucesor soportado es el fichero TypeScript que evalúa la CLI. Se eligió esta vía sobre los clics en el panel por la misma razón que todo lo demás en este proyecto: la definición del despliegue queda versionada, revisable en diff y reproducible, y `railway config plan` enseña exactamente qué se va a crear antes de tocar nada.
   ...
   - **No contiene secretos y por eso se versiona**: la base de datos se declara con `database("postgis", "postgres", { image: "postgis/postgis:17-3.5" })`, que es un Postgres con nuestra imagen y con credenciales generadas y custodiadas por Railway; la aplicación recibe `PGHOST`…`PGPASSWORD` como **referencias** (`db.env.PGPASSWORD`), no como valores. El plan las muestra redactadas. **Nunca usar `railway config pull --include-variables`**, que descifra los valores y los escribe en el fichero.
   - **En IaC, lo que no está en el fichero se borra**: cualquier servicio futuro se añade ahí, no por el panel.
   - **El volumen de la base de datos lo aprovisiona Railway**, con los mismos valores por defecto que desde el panel; vía IaC solo se puede ajustar la región, así que `volumeAttachments: null` en el plan es lo esperado y no una base de datos efímera.
...
- El despliegue efectivo depende de credenciales que la sesión no tiene: el MCP de Railway configurado no conectó (`CONNECTION_CLOSED`) y no hay CLI de Railway instalada. `docs/despliegue.md` deja el procedimiento manual completo.
- En cuanto el servicio quede arriba empieza a acumularse la primera serie de instantáneas, que es la condición para abrir la ADR de la categoría observada (ADR-005 §6) y para afinar `zaragoza.catalog.freshness.*` y la retención de `raw_payload`.
