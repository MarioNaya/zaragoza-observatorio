...
8. **Ningún fichero de configuración de Railway en el repositorio.** `railway.json` está deprecado y los servicios nuevos no pueden usarlo, así que escribirlo sería introducir hoy algo que deja de leerse el 2026-12-01. Los ajustes del servicio (ruta de healthcheck `/actuator/health`, volumen, variables) se aplican en el panel y quedan documentados paso a paso en `docs/despliegue.md`. Si más adelante se instala la CLI, la vía soportada es `.railway/railway.ts`.
...
- **`railway.json` en el repositorio**: deprecado, cerrado a servicios nuevos y con fecha de caducidad (2026-12-01). Documentar el panel es honesto; escribir configuración muerta, no.
