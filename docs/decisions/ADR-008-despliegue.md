...
- El MCP de Railway configurado no conectaba (`CONNECTION_CLOSED`) porque **el login de la CLI había caducado**: el servidor remoto se autentica con esa sesión. Se creyó resuelto con `railway login` y `railway setup agent --remote`. El despliegue en sí se hizo con la CLI, sin necesidad del MCP. **Desmentido el 2026-09-08**: con `railway whoami` correcto el MCP seguía fallando con `CONNECTION_CLOSED`, así que la causa no era el login y sigue sin conocerse (`docs/ESTADO.md` §6). No bloquea nada: la CLI cubre todo, incluida la API pública.
