...
- **El gasto público no es territorializable** (S0.2, S0.6, ADR-003). **Pero lo que se cayó fue el dinero, no el territorio**: 323 de 436 fichas (74 %) tienen señal territorial y la API resuelve dirección → junta con el recurso `portal`. Siguen en pie cruces con volumen real —`registro-licencia` (42.321 locales), `locales-vacios` (3.824, ya con su junta), `licencia-obra` (2.042 parcelas), `via-publica`, quejas (~50 % geolocalizadas)— con padrón y SOCIO24 de denominador. El producto es un observatorio territorial al que le falta una pata, no un proyecto sin eje (SPEC.md §1).
...
2. **Copias del volumen — único riesgo presente, no futuro.** Esta documentación declara en tres sitios que la serie de instantáneas «no se puede rehacer», y hoy **no está respaldada**: el activo señalado como irrepetible es el único sin protección. Railway admite `backupSchedules` (`DAILY`/`WEEKLY`/`MONTHLY`) en el volumen, y el tipo `VolumeMount` de la SDK lo acepta, así que se declara en `.railway/railway.ts` y se aplica con `railway config apply`. Son diez minutos y cierra la incoherencia.
...
Después viene la **fase 2** (`geo` y `citizen`, SPEC.md §3). El orden importa:

1. **Spike S2.1: resolución dirección/punto → junta**, antes de modelar nada. S0.6 dice que la API lo resuelve con el recurso `portal` y que varios datasets ya traen la junta (`locales-vacios.portal.junta`, `edificio-historico.junta`), pero **no está comprobado**. Es la llave del eje territorial entero: si esa resolución es peor de lo esperado, degradan a la vez las quejas, la actividad urbana y todos los cruces posteriores. Incluye la correspondencia `distrito.id` ↔ `idpadron` (SPEC.md §9) y medir el porcentaje sin asignar de cada fuente.
2. `geo`: `District` con geometría PostGIS (S0.4) y padrón por junta y año.
3. `citizen`: ingesta de `quejas-sugerencias/list.json` (S0.3), con la **decisión previa por ADR sobre el texto libre** (SPEC.md §9); el origen lleva datos personales de terceros sin anonimizar.

**Sobre el ritmo**: cada fuente nueva son spike, fixtures, ADR, traductor, migración y tests. Tres fuentes han costado cinco sesiones a esta calidad. Es preferible **dos o tres cruces bien hechos** —`registro-licencia`, `locales-vacios` y quejas, los tres con padrón, ya dan un producto defendible— que ocho fuentes a medias.
