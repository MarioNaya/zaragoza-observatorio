...
Última actualización: 2026-09-07, cierre de la quinta sesión (despliegue hecho: ADR-008, `docs/despliegue.md`, `.railway/railway.ts`). **La fase 1 está cerrada y la instancia está en marcha** en <https://observatorio-production-ed20.up.railway.app>, ingiriendo a diario. Lo que queda ya no es escribir código, sino esperar a que se acumule la primera serie de instantáneas (§4). Este documento es el punto de entrada de cada sesión de trabajo: qué está hecho, qué decisiones rigen, cómo se arranca el entorno y cuál es el siguiente paso concreto. Se actualiza al cerrar cada sesión.
...
| Fase 1: primera serie de instantáneas observadas | En curso (se acumula sola) | §4 |
...
3. **Vigilar el primer despliegue automático**: el servicio construye desde `main`, así que el próximo push a `main` redespliega. Conviene comprobar que un cambio con migración nueva arranca bien (`ddl-auto=validate` falla al arrancar, no en caliente).
