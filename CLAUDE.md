...
## Reglas adicionales (ADR-001 a ADR-005)
...
Con la app arrancada: `/actuator/health`, `/actuator/modulith`, `/api/v1/catalog/summary`, `/api/v1/catalog/datasets?sort=observedLastChange,desc`, `/v3/api-docs`, `/swagger-ui.html`. El planificador ingiere el catálogo real 30 s después de arrancar y a los 2 min empieza a observar distribuciones por lotes de 60 cada 10 min (`zaragoza.catalog.observation.*`; para todo el catálogo de una vez: `--zaragoza.catalog.observation.initial-delay=PT50S --zaragoza.catalog.observation.batch-size=500`, ~3,5 min).
