...
**geo** (S0.4, S2.1, ADR-011): `District` (junta municipal o vecinal: `id` = `distrito.id` = `iddatosab`, `padronId` = `idpadron` para cruzar con SOCIO24, nombre, tipo `municipal`/`vecinal`, geometría WGS84), `CensusSection` (`CUSEC`, geometría, junta) **—pendiente: no implementada—**, `PopulationRecord` (unidad territorial, año, total, españoles, extranjeros, menores, hogares y las desagregaciones que ofrezca la fuente; años 2020, 2021, 2022 y 2024, **sin 2023**: toda normalización declara el año que usa). Servicios `locate(point) -> District` (`ST_Contains` sobre los 29 polígonos; devuelve `resuelto`, `fuera` del término o `sin punto`, y marca la ambigüedad donde los polígonos se solapan) y, cuando existan las secciones, `locate(point) -> CensusSection`. Los nombres de junta se casarán por clave normalizada más una tabla explícita de sinónimos (`DISTRITO SUR`, `SAN JUAN DE MOZARRIFAR`): **decidido en ADR-011 y todavía no implementado**, se crea con `citizen`, que es la primera fuente que trae el nombre como texto. **La junta nunca se pide a la API municipal**: `portalero/v2` acierta el 89,5 % sin permitir distinguir el acierto del fallo y no tiene consulta espacial (S2.1). No existe `Neighbourhood`: los barrios no son dato abierto.

**citizen** (S0.3): `ServiceRequest` (id origen, código y nombre de servicio, título, descripción, estado, `requestedAt`, `closedAt` = `updated_datetime` solo en cerradas, punto WGS84 nullable, dirección textual nullable, `districtDeclared` = el `district` textual del origen tal cual, nullable —solo lo trae el 38 % y discrepa del punto en el 8,8 %—, `districtId` resuelto por geometría, nullable; el declarado nunca sustituye al resuelto, ADR-011), `Category` (taxonomía `services.json` tal cual más la jerarquía `parent` de `statistics`). No hay canal de entrada en los datos. **Datos personales**: el ayuntamiento publica `title` y `description` sin anonimizar y contienen nombres, firmas y DNI (comprobado el 2026-09-06). El observatorio no los republica tal cual: en fase 2 se decide si se almacenan redactados, truncados o solo como categoría y longitud (§9), y la API nunca expone el texto original.
