...
- `GET /citizen/requests` (filtros `district` —la junta **resuelta**—, `serviceCode`, `status`, `assignment`, `from`, `to`; `sort` en `requestedAt|updatedAt|id` con desempate estable por id) · `GET /citizen/aggregations?by=district|category|month` (cada grupo con su padrón, el año usado, las quejas por mil habitantes y la **cobertura de punto** del grupo; la respuesta con el reparto por estado de asignación y el total sin asignar) · `GET /citizen/summary` (totales, rango temporal, reparto por estado y contraste entre junta resuelta y declarada). **Implementados** (fase 2, S2.2, ADR-012). El eje `census-section` no se ofrece: las secciones censales no están implementadas y no se publica un eje que no se puede calcular.
