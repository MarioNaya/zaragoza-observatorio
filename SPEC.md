...
Versión 0.11 · 7 de septiembre de 2026 · Documento de arranque para trabajar con Claude Code. Revisado con los resultados de los spikes S0.1–S0.6 y S1.1–S1.3 (`docs/spikes/`), con las ADR-001 a ADR-009 (`docs/decisions/`), con la implementación completa de la fase 1 (`ingestion` y `catalog` con los dos ejes de frescura, el inventario de endpoints del Swagger y la federación en datos.gob.es) y con la **instancia ya desplegada y su coste acotado** (ADR-008, ADR-009, `docs/despliegue.md`). La v0.11 recoge además la **revisión de propósito** tras los spikes: qué eje territorial sobrevive a la caída de la hipótesis inversión↔quejas (§1), el alcance exacto del principio de no-conclusión (§1.1, §8 regla 6) y lo que eso implica para las fases 2 y 4 (§3). Estado del proyecto y arranque de sesión: `docs/ESTADO.md`. Diagramas: `docs/arquitectura.md`.
...
- **Copia de seguridad del volumen de producción**: la serie de instantáneas se declara irrepetible en tres sitios de esta especificación y hoy **no está respaldada**. Railway admite `backupSchedules` (`DAILY`/`WEEKLY`/`MONTHLY`) en el volumen y no se ha declarado en `.railway/railway.ts`. Es el único riesgo presente, no futuro, del despliegue.
