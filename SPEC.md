...
- **Despliegue** (ADR-008, procedimiento en `docs/despliegue.md`): imagen propia desde el `Dockerfile` de la raíz (JRE 21, usuario no root, capas de Boot) sobre **Railway**, con la **plantilla PostGIS `postgis/postgis:17-3.5`** —la misma imagen que desarrollo y Testcontainers— y volumen persistente, porque la serie histórica de instantáneas no se puede rehacer. Conexión por variables `PG*` sin valor por defecto: si falta una, la aplicación no arranca. **Una sola réplica** mientras no exista ShedLock (ADR-004). En producción el actuator expone solo `health` e `info`; `/actuator/modulith` no se expone, y `/v3/api-docs` y `/swagger-ui.html` sí, porque el contrato es parte del producto (§6). La imagen se prueba en local con `compose.prod.yaml` antes de publicar.
