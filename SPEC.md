...
**geo** (S0.4, S2.1, ADR-011): `District` (junta municipal o vecinal: `id` = `distrito.id` = `iddatosab`, `padronId` = `idpadron` para cruzar con SOCIO24, nombre, tipo `municipal`/`vecinal`, geometría WGS84), `CensusSection` (`CUSEC`, geometría, junta), `PopulationRecord` (unidad territorial, año, total, españoles, extranjeros, menores, hogares y las desagregaciones que ofrezca la fuente; años 2020, 2021, 2022 y 2024, **sin 2023**: toda normalización declara el año que usa). Servicios `locate(point) -> District` (`ST_Contains` sobre los 29 polígonos; devuelve `resuelto`, `fuera` del término o `sin punto`, y marca la ambigüedad donde los polígonos se solapan) y `locate(point) -> CensusSection`. Los nombres de junta se casan por clave normalizada más una tabla explícita de sinónimos (`DISTRITO SUR`, `SAN JUAN DE MOZARRIFAR`). **La junta nunca se pide a la API municipal**: `portalero/v2` acierta el 89,5 % sin permitir distinguir el acierto del fallo y no tiene consulta espacial (S2.1). No existe `Neighbourhood`: los barrios no son dato abierto.
