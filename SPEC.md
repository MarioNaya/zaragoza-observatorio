...
Versión 0.17 · 9 de septiembre de 2026 · Documento de arranque para trabajar con Claude Code. Revisado con los resultados de los spikes S0.1–S0.6, S1.1–S1.3 y S2.1–S2.3 (`docs/spikes/`), con las ADR-001 a ADR-015 (`docs/decisions/`), con la implementación completa de la fase 1 (`ingestion` y `catalog` con los dos ejes de frescura, el inventario de endpoints del Swagger y la federación en datos.gob.es), con los **módulos `geo` y `citizen` de la fase 2** (las 29 juntas con geometría, el padrón por año y la resolución punto → junta por `ST_Contains`, ADR-011; y las quejas y sugerencias ingeridas **sin su texto libre**, ADR-012) y con la **instancia desplegada, su coste acotado y su volumen respaldado** (ADR-008, ADR-009, ADR-010, `docs/despliegue.md`). La v0.11 recogió la **revisión de propósito** tras los spikes: qué eje territorial sobrevive a la caída de la hipótesis inversión↔quejas (§1), el alcance exacto del principio de no-conclusión (§1.1, §8 regla 6) y lo que eso implica para las fases 2 y 4 (§3). Estado del proyecto y arranque de sesión: `docs/ESTADO.md`. Diagramas: `docs/arquitectura.md`.
