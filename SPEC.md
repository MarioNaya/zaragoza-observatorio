...
**Lo que se cayó fue el dinero, no el territorio.** Conviene tenerlo presente para no arrastrar una promesa que los spikes ya desmintieron: de las 436 fichas, **323 (74 %) tienen alguna señal territorial** y 55 pasan las tres preguntas de S0.6, y la API resuelve dirección → junta con el recurso `portal`. Quedan cruces con volumen real y contrastado: `registro-licencia` (**42.321 locales** con geometría, epígrafe y año de licencia), `locales-vacios` (3.824, que **ya traen su junta**), `licencia-obra` (2.042 parcelas datadas), `via-publica`, quejas geolocalizadas (~50 %), y padrón y SOCIO24 como denominadores. Es decir: un observatorio territorial al que le falta **una** de sus cuatro patas, no un proyecto sin eje.
...
- **Spike previo S2.1, antes de modelar nada: resolución dirección/punto → junta.** S0.6 apunta que la API la resuelve con el recurso `portal`, y varios datasets ya traen la junta incorporada (`locales-vacios.portal.junta`, `edificio-historico.junta`, `arte-publico.barrio`), pero no está comprobado. **Es la llave de todo el eje territorial**: de su calidad dependen a la vez las quejas, la actividad urbana y cualquier cruce posterior, así que si degrada, degradan todos juntos. Incluye resolver la correspondencia `distrito.id` ↔ `idpadron` (§9) y medir qué porcentaje de cada fuente queda sin asignar.
