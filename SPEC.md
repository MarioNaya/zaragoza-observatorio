...
- **Herramienta de cruce, con una decisión de API que merece ADR propia antes de la primera pantalla.** Si el análisis lo hace el usuario (§1.1), el backend no puede exponer vistas precocinadas: necesita **primitivas componibles** —dimensión territorial, medida, denominador, ventana temporal— que el frontend combine. Sigue valiendo la regla 8 (agrega el backend, no el navegador), pero la agregación la parametriza **el usuario**, no el programador. Cuánta libertad se abre en esos parámetros, y cómo se acota sin volver a precocinar, es lo que hay que decidir por ADR.
- **La calidad del dato viaja con el cruce.** El eje observado de la fase 1 deja de ser un producto aparte y pasa a ser la **capa de credibilidad** de todo análisis: si alguien cruza dos capas y una lleva meses sin cambiar, el resultado —y su exportación— lo lleva pegado, no en otra pantalla. Es lo que distingue esta herramienta de un portal de datos: aquí se ve, mientras se analiza, hasta qué punto puede uno fiarse de cada capa.
...
├── territory/      # composición de lectura: ficha por junta (sin estado propio)
...
Las reglas 16 a 31 (spikes como tests JUnit, stack real, comportamiento verificado de la API municipal, unidad territorial, contexto `spending`, infraestructura de la fase 1, datos personales en fixtures, eje observado, inventario de la API, federación, despliegue, memoria y coste, copias de seguridad, resolución territorial, texto libre de las quejas y fichas dadas de baja del catálogo) derivan de las ADR y de los spikes y viven solo en `CLAUDE.md`.
...
- **Superficie de la API de cruce (fase 4)**: hasta dónde se parametrizan las agregaciones para que el usuario componga su análisis sin que el backend acabe precocinando vistas ni el navegador agregando (§3 fase 4, regla 8). ADR antes de la primera pantalla del frontend, junto con los valores por defecto, la clasificación de los mapas y los denominadores ofrecidos (§1.1).
