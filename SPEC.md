...
Versión 0.7 · 6 de septiembre de 2026 · Documento de arranque para trabajar con Claude Code. Revisado con los resultados de los spikes S0.1–S0.6, S1.1 y S1.2 (`docs/spikes/`), con las ADR-001 a ADR-006 (`docs/decisions/`) y con la implementación de la fase 1 (`ingestion` y `catalog` con los dos ejes de frescura y el inventario de endpoints del Swagger). Estado del proyecto y arranque de sesión: `docs/ESTADO.md`. Diagramas: `docs/arquitectura.md`.
...
2. `ingestion` consulta la fuente según el `SourceDescriptor`: URL siempre con `.json`/`.geojson`, `rows` (tope 500 en la sede), paginación por `start` (`OFFSET`) o una sola petición (`NONE`); un documento único (`DOCUMENT`, el Swagger de la API) se trae sin parámetros de paginación y cuenta como un registro (ADR-006). `If-Modified-Since` no se honra nunca y solo Open311 honra `If-None-Match` (S0.5): la incrementalidad va por filtros de fecha o por comparación con lo almacenado, decidida por cada fuente.
...
- `GET /catalog/datasets` (filtros `periodicity` [o `UNDECLARED`], `status`, `hasGeo`, `open`, `hasApi`, `freshness`, `observation`, `q`; `sort` en `title|id|issued|declaredModified|metadataUpdated|declaredRatio|observedLastChange`) · `GET /catalog/datasets/{id}` (ficha, distribuciones, última instantánea con ambos ejes, marca de la última observación y `apiEndpoints`: operaciones del Swagger bajo su `apiTag`, con `tagDocumented` y el `ingestedAt` del inventario) · `GET /catalog/datasets/{id}/freshness-history?limit=` · `GET /catalog/summary` (recuentos por categoría de frescura declarada, por periodicidad y por método de observación; con API, abiertos, explorables, con geo; `apiInventory`; umbrales vigentes) · `GET /catalog/api-tags` (unión de tags del Swagger y tags declarados, con operaciones y fichas de cada uno; orden `tag,asc`) · `GET /catalog/api-endpoints` (inventario paginado; filtros `tag`, `q`, `templated`; `sort` en `document|path|tag`). **Implementados** (fase 1, S1.2 incluido); contrato en `/v3/api-docs`.
...
- ~~Cruce `apiTag` → paths del Swagger (`sede/servicio/catalogo/api.json`)~~ → hecho el 2026-09-06 (S1.2, ADR-006). Marca `federated` (RDF/datos.gob.es): pendiente de la fase 1.
...
6. Cerrar la fase 1: ~~spike S1.1 (frescura observada) y su implementación~~ hecho el 2026-09-06 (ADR-005; comprobación real en `docs/ESTADO.md` §5); ~~cruce con el Swagger~~ hecho el 2026-09-06 (S1.2, ADR-006); queda `federated`; después, despliegue de una instancia.
