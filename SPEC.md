...
- **Fichas que desaparecen del catálogo** (detectado el 2026-09-08, primera vez que el listado encoge: 436 → 434). `catalog_dataset` solo hace upsert y no marca ni da de baja lo que deja de aparecer, así que `summary` sigue contándolas; la federación sí purga lo no visto (S1.3). El dato está en `lastSeenAt`: falta exponerlo. No se borra la ficha —su histórico de frescura es lo valioso—; decidir con ADR corta el filtro y el recuento, porque «desaparecida del catálogo» es una afirmación sobre el publicador (regla 6).
