...
Versión 0.9 · 6 de septiembre de 2026 · Documento de arranque para trabajar con Claude Code. Revisado con los resultados de los spikes S0.1–S0.6 y S1.1–S1.3 (`docs/spikes/`), con las ADR-001 a ADR-008 (`docs/decisions/`), con la implementación completa de la fase 1 (`ingestion` y `catalog` con los dos ejes de frescura, el inventario de endpoints del Swagger y la federación en datos.gob.es) y con la definición del despliegue (ADR-008, `docs/despliegue.md`). Estado del proyecto y arranque de sesión: `docs/ESTADO.md`. Diagramas: `docs/arquitectura.md`.
...
- ~~Plataforma de despliegue y exposición de `/actuator/modulith` en producción~~ → Railway con la plantilla PostGIS y una sola réplica; en `prod` el actuator expone solo `health` e `info` y springdoc queda visible (ADR-008, 2026-09-06). Abierto solo lo que depende de tener la instancia arriba: umbrales de frescura y retención de `raw_payload` con datos reales.
